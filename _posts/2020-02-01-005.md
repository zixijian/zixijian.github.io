...
推荐一款app：[【Batterychargelimit】](https://github.com/sriharshaarangi/BatteryChargeLimit)，  
可以控制安卓的充电策略：
- 充到设置的上限之后会停止充电；
- 掉电到下限后开始循环充电。
- 方便长期插电（已稳定运行600余天）。
...
为节省运行内存也可以停止安卓服务，  
使用 crontab 定时任务（详见[【Linux deploy 指南】](https://zixijian.github.io/2020/09/01/007.html)）。 

获取 root 权限后，安装 linuxdeploy ，部署 Debian9 arm64 版本（清华大学源）架构aarch64。  
吐血推荐安卓最好用的连接 ssh 软件 Juicessh（建议使用密钥连接）。
